...
・増加傾向 または 縮小傾向 を必ず最初に記載する。
・過去45日平均との差を金額と増減率(%)で必ず記載する。
・なぜ増減させたのかを売上推移・客数・曜日・祝日・イベント・気温・気温差・キャンペーン・他店舗傾向を用いて説明する。

例

増加傾向（過去45日平均比 +3,500円、+12.4%）
気温上昇とクール麺キャンペーン、直近3週の売上増加傾向が続いているため増額。

縮小傾向（過去45日平均比 -2,100円、-8.6%）
売上・客数ともに減少傾向で、キャンペーン効果も限定的なため減額。
...
・増減金額
・増減率(%)
増減率は対象商品の過去45日間の発注実績を基準として算出すること。

平均発注金額を算出する際は、
発注金額が存在しない日、null、空文字、0円のデータは平均計算から除外すること。

計算対象は発注金額が1円以上存在する日のみとする。

平均発注金額 =
有効な発注金額の合計 ÷ 有効データ件数

増減率 =
((今回発注金額 - 平均発注金額) ÷ 平均発注金額) × 100

有効データが存在しない場合は増減率を算出せず、
「比較対象データ不足」と記載すること。
